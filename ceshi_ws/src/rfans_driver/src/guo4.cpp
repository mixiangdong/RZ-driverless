...
spoint left1,left1_save, left2, right1, right2, middle1, middle2, middle2_, center,
    point[6], point_pair[3][2], temp_point, temp_left, temp_right,preview_point,now_location;;
double k0, k, k1, b, lateralerror, theta,aerfa,delta, a_fa, lateral, dis_point_point,Lm, a[6],
    bb[6],steering_wheel_angle_save_1, steering_wheel_angle_save_2,left1_left1_save_dis;
...
void callback2(const rfans_driver::point_zx::ConstPtr &point_msg) {
  ...
  */
  ...
  for (i2 = 0; i2 < 6; i2++) {
    ...
    for (i3 = i2 + 1; i3 < 6; i3++) {
      ...
      if (dis_point_point > 2.5 && dis_point_point < 3.2) {
  ...
  */
  if (left1.x != 1000 && left1.y != 1000 && left1_save.x != 1000 &&
    ...
    if (left1_left1_save_dis > 3&&left1.x<left1_save.x&&num>=1) {
      num--;
    }
  }
  left1_save = left1;
  ...
  */
  if (middle2.x == 1000 && middle2.y == 1000 && middle1.x != 1000 &&
    ...
    if (left1.y - right1.y == 0) {
     ...
     preview_point.x = (-1*k*b+sqrt(k*k*preview_L*preview_L+preview_L*preview_L-b*b))/(k*k+1);
     preview_point.y = k*preview_point.x+b;

   

      now_location.x = -(vL+0.9);
      now_location.y = 0;

      k1 = (preview_point.y - now_location.y) / (preview_point.x - now_location.x);
      aerfa = -atan(k1) / PI * 180;

      double Ld; //纯追踪后轴中心至预瞄点的距离
      Ld = sqrt((preview_point.x-now_location.x)*(preview_point.x-now_location.x)+
            (preview_point.y-now_location.y)*(preview_point.y-now_location.y));
      
      delta = atan(2*vL*sin(aerfa*PI/180)/Ld)*180/PI;

        ROS_INFO("delta: %.9f\n", 4*delta);

      steering_wheel_angle.data = 4*delta;
...
/*
    ...
    */
    ...
    Lm = sqrt((middle1.x*middle1.x)+(middle1.y*middle1.y));
    if(Lm > preview_L){

        preview_point.x = middle1.x;
        preview_point.y = middle1.y;
    }
    else{
...
/*
  ...
  */
  if (steering_wheel_angle.data > 100) {
    cout << steering_wheel_angle.data << endl;
    steering_wheel_angle.data = 100;
  } else if (steering_wheel_angle.data < -1 * 100) {
    cout << steering_wheel_angle.data << endl;
    steering_wheel_angle.data = -1 * 100;
  } else {
    steering_wheel_angle.data = steering_wheel_angle.data;
  }
