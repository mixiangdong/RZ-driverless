...
#define gps_vehicle_center_L 0.8
...
private:
  ...
  spoint left1, left1_, left2, left2_, right1, right1_, right2, right2_,
      middle1, middle1_, middle2, middle2_, now_location, now_location_,vehicle_center;
  ...
  int arrive_target_flag, target_vehicle_distance_flag_save, NUM=0,
 ...
 */
  ...
  void GeoToGauss(double jd, double wd, short DH, short DH_width, double *y,
    ...
    jd_hd = jd / 3600.0 * PI /
            180.0; // 将以秒为单位的经度转换成度为单位再转换为弧度
    wd_hd =
        wd / 3600 * PI / 180.0; // 将以秒为单位的纬度转换成度为单位再转换为弧度
  ...
  void callback1(const rfans_driver::gps_data::ConstPtr &gps_msg) {
    ...
    //合肥经度117.25，3度带是39带号
    GeoToGauss(n * 3600, m * 3600, 38, 3, &y, &x, 114);
    // now_location.x = gps_msg->x;
    // now_location.y = gps_msg->y;
    now_location.x = x;
    now_location.y = y;
    now_location.yaw = gps_msg->yaw;
 ROS_INFO("now_location.x: %.9f,now_location.y :%.9f,now_location.yaw:%.9f",now_location.x, now_location.y, now_location.yaw);
  ...
  /*	void callback2(const rfans_driver::lidar::ConstPtr &point_msg)
                  {
                          ...
                          ROS_INFO("left2.x: %f,left2.y: %f,right.x:
     %f,right2.y: %f\n",left2.x,left2.y,right2.x,right2.y);
                          ROS_INFO("left1.x: %f,left1.y: %f,right1.x:
  ...
  */
  ...
  bool callback3(rfans_driver::null::Request &req,
    ...
    target_vehicle_distance_flag_save = 1;
    target_vehicle_distance_flag = 1;
    disThreshold1 = 3; //迭代距离极限
    disThreshold2 = 2; //备用
    ...
    if (req.state) {
      ...
      for (;;) {
        ...
        if (arrive_target_flag == 1 && start_sign == 1 && left1_.x != 1000 &&
            left2_.x != 1000 && right1_.x != 1000 &&
            right2_.x !=
                1000) // start_sign为1说明回调函数第一次运行，假设车第一次到达目标点
        {
          start_sign = 0;
          arrive_target_flag = 0;

          a_middle.x = now_location_.x, a_middle.y = now_location_.y;
          b_left.x = left1_.x, b_left.y = left1_.y;
          b_right.x = right1_.x, b_right.y = right1_.y;
          c_left.x = left2_.x, c_left.y = left2_.y;
          c_right.x = right2_.x, c_right.y = right2_.y;
          ...
          NUM += 1;
        } else if ((arrive_target_flag == 1) && (start_sign == 0) &&
                   left1_.x != 1000 && left2_.x != 1000 && right1_.x != 1000 &&
                   right2_.x != 1000)

        {
          a_left.x = b_left.x, a_left.y = b_left.y;
          a_right.x = b_right.x, a_right.y = b_right.y;
          b_left.x = left1_.x, b_left.y = left1_.y;
          b_right.x = right1_.x, b_right.y = right1_.y;
          c_left.x = left2_.x, c_left.y = left2_.y;
          c_right.x = right2_.x, c_right.y = right2_.y;
          ...
          arrive_target_flag = 0;
          NUM += 1;
        } else if (left1_.x == 1000 || left2_.x == 1000 || right1_.x == 1000 ||
                   right2_.x == 1000) {
          ROS_INFO("the number of points is so less");
        } else {
        }
        ROS_INFO("NUM: %d\n", NUM);
...
{
target_radius=small_radius_factor*small_radius+large_radius_factor*large_radius;

                   middle_center.x=(small_center_factor*small_center.x+large_center_factor*large_center.x)/2;
                   middle_center.y=(small_center_factor*small_center.y+large_center_factor*large_center.y)/2;
}
               else
               {
                       target_radius=small_radius+1.5;
       ...
       */
          ...
          vehicle_center.x=now_location_.x+gps_vehicle_center_L*sin(now_location_.yaw*PI/180);
          vehicle_center.y=now_location_.y+gps_vehicle_center_L*cos(now_location_.yaw*PI/180);
          vehicle_center.yaw=now_location_.yaw;
          distance(vehicle_center, middle_center, &center_vehicle_distance);
          // ROS_INFO("center_vehicle_distance: %.9f\n",
          // center_vehicle_distance);
          cha = target_radius - center_vehicle_distance;
          cross_mult = cross_multiply((90 - vehicle_center.yaw), middle_center,
                                      vehicle_center);
          // ROS_INFO("cross_mult: %.9f\n", cross_mult);
          vector_multiply((90 - vehicle_center.yaw), middle_center,
                          vehicle_center, &multiply);
          ...
          control_factor1 =
              symbol_2 *
              fabs(angle_); //若为+，则应该控制车子逆时针转向，即左转;反之亦反.
          state = 1;
          ...
          // yaw_diff = angle_target - (90-vehicle_center.yaw);
          // //-时需要顺时针转向
          yaw_diff = cha_angle(angle_target, 90 - vehicle_center.yaw);
          control_factor1 = (-1 * yaw_diff); //改变方向，统一符号
          // ROS_INFO("yaw_diff: %.9f\n", yaw_diff);
          angle_assist = angle(a_middle.x, a_middle.y, c_middle.x, c_middle.y);
          ...
          double angle_c =
              angle(vehicle_center.x, vehicle_center.y, c_middle.x, c_middle.y);
          ROS_INFO("angle_c: %.9f\n", angle_c);
          distance(vehicle_center, c_middle, &now_location_c_middle_dis);
          lateralError =
        ...
        distance(c_middle, vehicle_center, &target_vehicle_distance);

        if (target_vehicle_distance <= disThreshold1)
          target_vehicle_distance_flag = 1;
        else
          target_vehicle_distance_flag = 0;
        ROS_INFO("target_vehicle_distance: %.9f\n", target_vehicle_distance);
        if (target_vehicle_distance_flag == 1 &&
            target_vehicle_distance_flag_save == 0)
          arrive_target_flag = 1;

        target_vehicle_distance_flag_save = target_vehicle_distance_flag;
        ...
        ROS_INFO("a_middle.x: %.9f a_middle.y: %.9f \n\n\n", a_middle.x,
                 a_middle.y);
        ROS_INFO("vehicle_center.x: %.9f,vehicle_center.y :%.9f,vehicle_center.yaw "
                 ":%.9f\n",
                 vehicle_center.x, vehicle_center.y, vehicle_center.yaw);
        ROS_INFO("state: %d\n\n\n", state);
        ...
        steering_wheel_angle.data =
            PID2_realize(&yaw_pid, 0.0, control_factor1) +
            PID2_realize(&lateralError_pid, 0.0, control_factor2);
  ...
  buff[0] = -1*steering_wheel_angle.data;
  // buff[1] = msg->y;
