...
public:
  ...
  void cloud_callback(const sensor_msgs::PointCloud2::ConstPtr &input) 
  {
    ...
    *cloud = *cloud_source_filtered_z;
    ...
    remove_close_pt(0.2, 15, cloud, remove_close);
    *cloud = *remove_close;
...
*/   
 ...
 //欧式聚类
    pcl::search::KdTree<pcl::PointXYZ>::Ptr tree_3d (new pcl::search::KdTree<pcl::PointXYZ>);;
    tree_3d->setInputCloud (cloud);
    std::vector<pcl::PointIndices> cluster_indices_3d;                      //创建点云索引向量，用于存储实际的点云信息
    pcl::EuclideanClusterExtraction<pcl::PointXYZ> ec_3d;	                //欧式聚类对象
    ec_3d.setClusterTolerance (0.1);								    	//设置近邻搜索的搜索半径，这里单位为m
    ec_3d.setMinClusterSize (5);								            //设置一个聚类需要的最少的点数目为2000
    ec_3d.setMaxClusterSize (1000);								      	    //设置一个聚类需要的最大点数目为25000
    ec_3d.setSearchMethod (tree_3d);								        //设置点云的搜索机制
    ec_3d.setInputCloud (cloud);					                        //输入需要聚类的点云指针地址
    ec_3d.extract (cluster_indices_3d);								      	//得到2000-25000点的聚类
    std::cerr << "聚类数量: " << cluster_indices_3d.size() << "\n" << std::endl;
    
    int cluster_count_3d = 0, cluster_count_small = 0;
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_cluster_total(new pcl::PointCloud<pcl::PointXYZ>);//保存锥桶点云
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_cluster_big(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_cluster_small(new pcl::PointCloud<pcl::PointXYZ>);
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices_3d.begin(); it != cluster_indices_3d.end(); ++it)
    {
        ...
        //std::cerr << "Cluster " << cluster_count_3d << " has " << cloud_cluster->points.size () << " data points." << std::endl;
        centroid_x = sum_x / cloud_cluster->points.size();
        centroid_y = sum_y / cloud_cluster->points.size();
        //std::cerr << "The centroid of cluster: x = " << x << ", y = " << y << std::endl;

        double distance;
        distance =  std::sqrt( std::pow(centroid_x, 2) + std::pow(centroid_y, 2) );
       ...
       if((max.z - min.z < 0.35) && (max.z - min.z > 0.05))//提取小锥桶,过滤其他物体，但会出现第三对锥桶闪的情况
      {
            ...
            else
            {
                    ...
                    std::cerr << "x = " << centroid_x << "; y = " << centroid_y << std::endl;
                    for (std::vector<int>::const_iterator pit = it->indices.begin(); pit != it->indices.end(); ++pit)
